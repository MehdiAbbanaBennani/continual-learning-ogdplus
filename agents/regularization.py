...
class L2(NormalNN):
    ...
    @article{kirkpatrick2017overcoming,
        ...
        author={Kirkpatrick, James and Pascanu, Razvan and Rabinowitz, Neil and Veness, Joel and Desjardins,
        Guillaume and Rusu, Andrei A and Milan, Kieran and Quan, John and Ramalho, Tiago and Grabska-Barwinska,
    ...
    def __init__(self, agent_config,
                 val_loaders, wandb_run):
        super().__init__(agent_config,
                         wandb_run=wandb_run, val_loaders=val_loaders)
...
def EWC_online(agent_config,
               val_loaders, wandb_run):
    agent = EWC(agent_config, wandb_run=wandb_run, val_loaders=val_loaders)
...
class SI(L2):
    ...
    def update_model(self, inputs, targets, tasks):
        ...
        for n, p in self.params.items():
            ...
            if n in unreg_gradients.keys():  # In multi-head network, some head could have no grad (lazy) since no
                # loss go through it.
    ...
    def update_model(self, inputs, targets, tasks):
        ...
        for n, p in self.params.items():
            delta = p.detach() - old_params[n]
            if p.grad is not None:  # In multi-head network, some head could have no grad (lazy) since no loss go 
            through it.
